          ...
          <div className="flex items-center">
            ...
            <ul className="hidden items-center lg:flex">
              {navItems.map(({ label, to }) => {
                const isActive = location == label.toLowerCase();
                return (
                  <li key={to} className="m-2">
                    <Link
                      to={to}
                      className={`relative text-ink transition-colors hover:text-primary-dark after:absolute after:-bottom-1 after:left-0 after:h-0.5 after:w-full after:origin-left after:scale-x-0 after:bg-primary-dark after:transition-transform after:duration-300 hover:after:scale-x-100 ${isActive && "text-primary-dark"}`}
                    >
                      {label}
                    </Link>
                  </li>
                  ...
                  return (
                    <li key={to}>
                      <Link
                        to={to}
                        onClick={closeMenu}
                        className={`flex items-center justify-between border-b border-line py-3 text-ink transition-colors hover:text-primary-dark ${isActive && "text-primary-dark"}`}
                      >
                        {label}
